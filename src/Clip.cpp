...
double Clip::resolve_timeline_fps() const
{
	if (timeline) {
		const Timeline* attached_timeline = dynamic_cast<const Timeline*>(timeline);
		if (attached_timeline) {
			double timeline_fps = attached_timeline->info.fps.ToDouble();
			if (timeline_fps > 0.0) {
				return timeline_fps;
			}
		}
	...
	double clip_fps = info.fps.ToDouble();
	if (clip_fps > 0.0) {
		return clip_fps;
	}

	if (reader) {
		double reader_fps = reader->info.fps.ToDouble();
		if (reader_fps > 0.0) {
			return reader_fps;
		}
	}
...
int64_t Clip::curve_extent_frames() const
{
	if (time.GetCount() <= 1) {
		return 0;
	}

	double max_timeline_frame = 0.0;
	for (int64_t index = 0; index < time.GetCount(); ++index) {
		const Point& curve_point = time.GetPoint(index);
		if (curve_point.co.X > max_timeline_frame) {
			max_timeline_frame = curve_point.co.X;
		}
	}

	if (max_timeline_frame <= 0.0) {
		return 0;
	}

	return static_cast<int64_t>(std::llround(max_timeline_frame));
}

int64_t Clip::trim_extent_frames(double fps_value) const
{
	if (fps_value <= 0.0) {
		return 0;
	}
	...
	const double trimmed_start_seconds = static_cast<double>(ClipBase::Start());
	const double trimmed_end_seconds = static_cast<double>(ClipBase::End());

	bool has_left_trim = trimmed_start_seconds > epsilon;
	double original_duration = static_cast<double>(info.duration);
	if (original_duration <= 0.0 && reader) {
		original_duration = static_cast<double>(reader->info.duration);
	}
	if (original_duration <= 0.0) {
		double source_fps = info.fps.ToDouble();
		int64_t source_length = info.video_length;
		if ((source_length <= 0 || source_fps <= 0.0) && reader) {
			source_length = reader->VideoLength();
			source_fps = reader->info.fps.ToDouble();
		}
		if (source_length > 0 && source_fps > 0.0) {
			original_duration = static_cast<double>(source_length) / source_fps;
		}
	}
	bool has_right_trim = false;
	if (original_duration > 0.0) {
		has_right_trim = std::fabs(trimmed_end_seconds - original_duration) > epsilon;
	}

	if (!has_left_trim && !has_right_trim) {
		int64_t base_frames = info.video_length;
		if (base_frames <= 0 && reader) {
			base_frames = reader->VideoLength();
		}
		if (base_frames > 0) {
			return base_frames;
		}
	...
	if (trimmed_end_seconds <= trimmed_start_seconds) {
		return 0;
	}

	const int64_t start_frame = static_cast<int64_t>(std::llround(trimmed_start_seconds * fps_value));
	const int64_t end_frame = static_cast<int64_t>(std::llround(trimmed_end_seconds * fps_value));
	const int64_t trimmed_frames = end_frame - start_frame;
	return trimmed_frames > 0 ? trimmed_frames : 0;
}
// Determine clip video length (frame count), accounting for time-mapping curves when present.
int64_t Clip::VideoLength() const
{
	double fps_value = resolve_timeline_fps();
	int64_t trim_frames = trim_extent_frames(fps_value);
	int64_t curve_frames = curve_extent_frames();

	int64_t timeline_frames = std::max(trim_frames, curve_frames);
	if (timeline_frames > 0) {
		return timeline_frames;
	}

	if (info.video_length > 0) {
		return info.video_length;
	}

	if (reader) {
		return reader->VideoLength();
	}

	return 0;
}

float Clip::MaxDuration() const
{
	double fps_value = resolve_timeline_fps();
	int64_t curve_frames = curve_extent_frames();

	if (fps_value > 0.0 && curve_frames > 0) {
		return static_cast<float>(static_cast<double>(curve_frames) / fps_value);
	}

	float fallback_duration = ClipBase::Duration();
	if (fallback_duration > 0.0f) {
	...
	double info_fps = info.fps.ToDouble();
	if (info.video_length > 0 && info_fps > 0.0) {
		return static_cast<float>(static_cast<double>(info.video_length) / info_fps);
	}
	...
	if (reader) {
		...
		double reader_fps = reader->info.fps.ToDouble();
		if (reader->info.video_length > 0 && reader_fps > 0.0) {
			return static_cast<float>(static_cast<double>(reader->info.video_length) / reader_fps);
		}
	}
